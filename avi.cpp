...
void registrarVuelo() {
    ...
    cout <<"tiene alguna dicapacida?(0:si / 1:no): ";
    cin >> PasajeroNuevo->dicapacidad;
...
void dicacpa(avion *aux){

    if(aux != NULL){
        
        dicacpa(aux->izq);

        cout<<"vuelo: "<<aux->ciudadOri<<" - "<<aux->destino<<endl;
        cout<<"fecha: "<<aux->dia<<"/"<< aux->mes<<"/"<<aux->anio<<"- hora de despegue: "<<aux->hora<<endl;
        cout<<"numero vuelo: "<<aux->Nvuelo<<endl;
        cout<<"pasajeros con cupos normales: "<<aux->cupoNormal<<endl;
        cout<<"pasajeros con cupos de sobreventa: "<<aux->sobreVenta<<endl;

        if(aux->primePasajeros){
            cout<<"pasajeros con dicapacidad registrados: "<< endl;
            pasajeros *auxP = aux->primePasajeros;
            while (auxP != NULL ) {
                if(auxP->dicapacidad == 0) {
                    cout << "- " << auxP->nombre << endl;
                }
                auxP = auxP->sig;
            }
            ...
            cout<<"no hay pasajeros con discapacidad registrados: "<< endl;
        }
        cout<<"-------------------------------"<< endl<<endl;
        dicacpa(aux->der);
